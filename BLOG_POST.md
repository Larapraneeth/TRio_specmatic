...
1. **Write an OpenAPI spec for the Ollama API** (`mock/ollama_api.yaml`) describing the
   endpoints my backend calls — `/api/chat`, `/api/generate`, and `/api/tags` — and the
   shape of their responses.
2. **Run Specmatic in stub (mock) mode** against that spec. This starts a fake Ollama on
...
3. **Point the backend at the mock** by setting `OLLAMA_BASE_URL=http://localhost:8888`.
...
`runOptions`), with resiliency enabled via `specmatic.settings.test.schemaResiliencyTests:
all`. The property nesting is strict, and the parser errors themselves were the best
...
## Part 3: Review-driven refinements — faithful mocks, separated reports, full coverage

The last round of review pushed on three things that turned out to be genuinely
instructive.

### Make a mock faithful to the real contract, not just convenient

My first Ollama mock worked, but I had quietly renamed schema components and changed
which fields were required to suit my project. The reviewer rightly pushed back: a mock
of a real service should mirror that service's *actual* contract, or it stops being a
trustworthy stand-in. I reworked `mock/ollama_api.yaml` to use the official Ollama
component names and required fields — `ChatRequest`, `ChatMessage`, `ChatResponse`,
`GenerateRequest`/`Response`, `ListResponse`, `ModelSummary`, `ModelOptions` — exactly
as the upstream spec defines them. The only liberties I kept are ones that don't break
fidelity: including just the endpoints TRIO calls, omitting unused fields, and adding
examples for mocking. I documented those deviations in `mock/OLLAMA_MOCK_NOTES.md` so the
reasoning is explicit. The lesson: trimming a contract is fine; *rewriting* its shape
quietly is not.

### Contract vs resiliency are different runs, and should look different

I had been treating one Specmatic run as both the contract and the resiliency report, so
the two were identical. They are conceptually different: the contract run checks the
documented happy paths; the resiliency run additionally generates negative and mutated
requests. I separated them — the contract run uses the standard tests, the resiliency run
enables generation via `SPECMATIC_GENERATIVE_TESTS` — and the CI now produces two distinct
report artifacts. Seeing the two reports differ in size and content is itself a useful
signal that the suites are doing different jobs.

### Cover the negatives explicitly, don't rely on generation alone

The contract report still showed a few skipped tests: the `422` validation paths weren't
exercised in the pure contract run because the only examples I had were happy-path ones.
The fix was to add paired *negative* examples — a request that intentionally violates the
schema (a wrong-typed required field) tied to the `422` response body — for each affected
endpoint. With those in place the contract run covers the error paths too, and the report
shows no skipped tests. Generation will find these cases automatically in resiliency mode,
but documenting them as examples makes the contract itself the place where the expected
error behaviour lives.

### Closing thought

Across these rounds the throughline has been the same: the spec is not paperwork bolted on
after the code — it is the contract the code must honour, the script the tests run from, and
the definition the mocks must stay true to. Each piece of review feedback was, in effect, a
nudge to make the spec carry more of that weight honestly.
